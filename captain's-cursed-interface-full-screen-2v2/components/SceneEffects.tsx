...
const mistFragmentShader = `
  ...
  void main() {
    vec2 uv = vUv;
    uv.x *= uAspect; // Dynamic aspect fix
    ...
    // Vignette & Edge fade (adapted for aspect)
...
const BaseEffect: React.FC<EffectProps & { fragmentShader: string, uniformData: any }> = ({ 
  ...
  const { viewport } = useThree(); // Use viewport to get dimensions of the canvas
  ...
  return (
    <mesh ref={meshRef}>
      {/* Plane automatically covers the viewport */}
